...
// Empêcher la sélection du même praticien comme remplaçant
document.getElementById('PRA_NUM').addEventListener('change', function() {
    const praNum = this.value;
    const remplacantSelect = document.getElementById('PRA_NUM_REMPLACANT');
    
    // Si le remplaçant actuel est le même que le nouveau praticien sélectionné, on le réinitialise
    if (remplacantSelect.value === praNum && praNum !== "") {
        remplacantSelect.value = "";
    }
    
    // Désactiver l'option correspondante
    Array.from(remplacantSelect.options).forEach(option => {
        if (option.value !== "") {
            option.disabled = (option.value === praNum);
        }
    });
});
...
document.getElementById('formRapport').addEventListener('submit', function(e) {
    ...
    // Vérifier que le praticien remplaçant est différent
    const praNum = document.getElementById('PRA_NUM').value;
    const praNumRemplacant = document.getElementById('PRA_NUM_REMPLACANT').value;
    if (praNum && praNumRemplacant && praNum === praNumRemplacant) {
        erreurs.push("Le praticien remplaçant ne peut pas être le même que le praticien visité.");
    }
